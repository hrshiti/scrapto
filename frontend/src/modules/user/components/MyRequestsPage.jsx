...
const MyRequestsPage = () => {
  ...
  const transformOrder = (order) => {
    ...
    const categories = [...new Set(order.scrapItems?.map(item => {
    ...
    // Format address
    const addressParts = [];
    if (order.pickupAddress?.street) addressParts.push(order.pickupAddress.street);
    if (order.pickupAddress?.city) addressParts.push(order.pickupAddress.city);
    if (order.pickupAddress?.state) addressParts.push(order.pickupAddress.state);
    if (order.pickupAddress?.pincode) addressParts.push(order.pickupAddress.pincode);
  ...
  const fetchOrders = useCallback(async () => {
    try {
      setError(null);
      const queryParams = selectedFilter !== 'all' ? `status=${selectedFilter === 'accepted' ? 'confirmed' : selectedFilter}` : '';
      ...
      <div className="px-4 md:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
              {[
                ...
                { label: 'On the Way', value: requests.filter(r => r.status === 'in_progress').length, color: '#8b5cf6' },
        ...
        {loading && (
          <motion.div
            ...
            style={{ backgroundColor: '#ffffff' }}
          >
            <FaSpinner className="animate-spin mx-auto mb-4" size={32} style={{ color: '#64946e' }} />
            <p style={{ color: '#718096' }}>Loading your requests...</p>
          </motion.div>
        )}
        ...
        {!loading && error && (
          <motion.div
            ...
            style={{ backgroundColor: '#ffffff' }}
          >
            <FaTimesCircle size={48} className="mx-auto mb-4" style={{ color: '#ef4444' }} />
              ...
              filteredRequests.map((request, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(request.status === 'accepted' || request.status === 'in_progress') && request.scrapperName && (
                      <div className="flex gap-2 mt-3 pt-3 border-t" style={{ borderColor: 'rgba(100, 148, 110, 0.15)' }}>
                        <button
                    ...
                    {request.status === 'completed' && request.scrapperName && (
                      <div className="flex gap-2 mt-3 pt-3 border-t" style={{ borderColor: 'rgba(100, 148, 110, 0.15)' }}>
                        {request.hasReview ? (
                          ...
                          <button
                            onClick={() => navigate(`/review-order/${request.id}`)}
                            className="flex-1 py-2 px-4 rounded-lg text-sm font-semibold text-white text-center transition-all flex items-center justify-center gap-2"
                            style={{ backgroundColor: '#f59e0b' }}
                            onMouseEnter={(e) => {
