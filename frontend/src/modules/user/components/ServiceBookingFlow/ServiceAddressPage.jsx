...
const ServiceAddressPage = () => {
    ...
    const getCurrentLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                setAddress(`Lat: ${coords.lat.toFixed(6)}, Lng: ${coords.lng.toFixed(6)}`); // Placeholder reverse geocode
            },
            (error) => {
                setIsGettingLocation(false);
    ...
    const handleContinue = () => {
        if (!address.trim() || !coordinates) {
            alert('Please provide a valid address and location.');
            return;
        }
        ...
        navigate('/book-service/schedule');
    };
    ...
    return (
        ...
        >
            <div className="flex items-center gap-4 p-4 border-b bg-white/80 backdrop-blur-md sticky top-0 z-10">
                ...
                <h1 className="text-lg font-bold text-gray-800">Pickup Address</h1>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 pb-24">
                <div className="bg-white p-4 rounded-xl shadow-sm mb-6">
                    ...
                    >
                        {isGettingLocation ? 'Locating...' : <><FaLocationArrow /> Get Current Location</>}
                    </button>
